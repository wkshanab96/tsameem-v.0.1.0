...
export const initializeStorage = async () => {
  ...
  try {
    ...
    // Call the API to ensure the extracted_text column exists
    try {
      console.log("Checking if extracted_text column exists...");
      const response = await fetch("/api/add-extracted-text-column", {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        console.log("extracted_text column check result:", result);
      } else {
        console.warn(
          "extracted_text column check API failed, continuing anyway",
        );
      }
    } catch (setupError) {
      console.warn(
        "Error calling extracted_text column check API:",
        setupError,
      );
      // Continue anyway
    }
    ...
    if (listError) {
      ...
      console.warn("Continuing despite error listing buckets");
    }
    ...
    if (!bucketExists) {
      ...
      try {
        ...
        if (rpcError) {
          ...
          console.log(`Bucket created successfully via RPC: ${BUCKET_NAME}`);

          // Apply SQL migration to fix permissions
          try {
            ...
            if (response.ok) {
              console.log("Storage permissions setup successful");
            } else {
              console.warn(
                "Storage permissions setup failed, continuing anyway",
              );
            }
          } catch (setupError) {
            console.warn("Error setting up storage permissions:", setupError);
          }
      ...
      try {
        ...
        if (error) {
          ...
          try {
            ...
            if (minimalError) {
              ...
              console.log(
                `Bucket created successfully with minimal options: ${BUCKET_NAME}`,
              );
            }
          } catch (minimalError) {
          ...
          console.log(
            `Bucket created successfully via direct API: ${BUCKET_NAME}`,
          );
        }
        ...
        try {
          ...
          if (response.ok) {
            ...
            console.warn("Storage permissions setup failed, continuing anyway");
          }
        } catch (setupError) {
          console.warn("Error setting up storage permissions:", setupError);
        }
        ...
        try {
          ...
          if (error) {
            console.error("Error creating bucket with minimal options:", error);
            // Continue anyway instead of throwing
            console.warn(
          ...
          console.error("Final bucket creation attempt failed:", finalError);
          // Continue anyway instead of throwing
          console.warn(
            "Continuing despite all bucket creation attempts failing",
          );
    ...
    if (!bucketVerified) {
      ...
      try {
        ...
        if (updateError) {
          ...
          // This allows the application to function even if bucket update fails
        ...
        .then((processedData) => {
          ...
          if (processedData) {
            ...
            supabase
              ...
              .eq("id", fileId)
              .then((result) => {
                console.log("File updated with processed data:", result);
              })
              .catch((error) => {
                console.error(
                  "Error updating file with processed data:",
                  error,
                );
              });
          }
